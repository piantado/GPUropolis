...
void device_run(int N, int PROPOSAL, int MCMC_ITERATIONS, float POSTERIOR_TEMPERATURE, int DLEN, float* device_INPUT, float* device_OUTPUT, float* device_SD, hypothesis* out_hypotheses, hypothesis* out_MAPs, int myseed, int initialize_sample)
{
	
	float stack[MAX_PROGRAM_LENGTH+1]; // Create the stack, allowing for one constant for each op to be passed (really we only need half this, right?)
	...
	for(int idx=0;idx<N;idx++) {
		...
		if(initialize_sample) { // randomly initialize if we should
			...
			compute_length_and_proposal_generation_lp(current);
		}
		else { // Else copy over
			memcpy((void*)current, out_hypotheses + idx, sizeof(hypothesis));
		}
		
		compute_posterior(DLEN, device_INPUT, device_OUTPUT, device_SD, current, stack);
		
		// Set some check bits to catch overflows/underflows, etc. If any change, we have a problem!
		current->check0 = 33; current->check1 = 33; current->check2 = 33; current->check3 = 33;
		
		// initialize proposal and MAP
		...
		for(int mcmci=0;mcmci<MCMC_ITERATIONS;mcmci++) {
			for(int pp=0;pp<=4;pp++) { // for each kind of proposal
				...
				if(pp==0 && (PROPOSAL & (0x1<<0)) ) { // full regeneration -- NOTE: THESE ARE ALMOST NEVER ACCEPTED SO WE NOW DON'T USE THEM!
					...
					int pos = MAX_PROGRAM_LENGTH-1 - random_int(current->program_length-1, RNG_ARGS);
					...
					int cntidentical = count_identical_to(below_cut, insertedWithBelow, MAX_PROGRAM_LENGTH-1); // how many subtrees of our replacement are equal to the bit we saved?
					...
					int subpos = MAX_PROGRAM_LENGTH-1 - random_int( below->program_length, RNG_ARGS);
				...
				if(ran) {
					...
					compute_posterior(DLEN, device_INPUT, device_OUTPUT, device_SD, proposal, stack);
