/*
 * TODO:
 * 	- RE-ENABLE RANDOMIZATIONF ROM TIME
 * 
...
const int MCMC_ITERATIONS = 1; // 6000000 = 7hrs; //50000;
...
__device__ void special_splice(int* ar, int start_ar, int end_ar, int* x, int start_x, int* dest) {
	...
	// TODO: THIS MAY NOT BE WORKING RIGHT YET....
	
	
	
	// correct for the mis-alignment of x and the gap of what its replacing
	int shift = (MAX_PROGRAM_LENGTH - start_x) - (end_ar-start_ar); 
	 
	int xi = start_x;
	for(int ari=0;ari<MAX_PROGRAM_LENGTH;ari++) {
		int in_splice_region = (ari+shift>=start_ar) && (ari+shift<end_ar);
		...
		// TOOD: WE ARE HERE TRYING TO FIGURE OUT HOW TO COPY.
		// LOOK AT ORIGINAL AND PROPOSAL... ARE WE GOING IN THE WRONG DIRECTION??
		/*
		 * Oh I see. The interpreter goes L->R. So when I count up "closes" I should also go L->R
		 * and 
		 * 
		 */
		// wrap this for in case it goes off the end
		int ar_ari_shift = -1;// ar[ari+shift];//ifthen( ari+shift < 0 || (!in_final_region && ari+shift >= MAX_PROGRAM_LENGTH), 0, ar[ari+shift]); 
...
__global__ void device_run(int N, int seed, float* out_posteriors, int* out_programs)
{
	...
	for(int mcmci=0;mcmci<MCMC_ITERATIONS;mcmci++) {
		...
		int end_ar = MAX_PROGRAM_LENGTH - random_int(len, rx,ry,rz,rw); // the position of what we replace
		...
		// And insert this where we wanted:
		special_splice(program, start_ar, end_ar, program_buf, start_x, program_proposal);

		 
		// This just samples from the prior for a proposal:
		// and works best on simple cases
// 		random_closed_expression(program_proposal, rx, ry, rz, rw);
// 		float fb = 0.0; // for using just generate random proposals
...
// 		}
		...
		float fb = (-log(current_len)-proposed_len*log(float(NUM_OPS))) - (-log(proposed_len)-current_len*log(float(NUM_OPS)));
...
// 			program[i] = ifthen(swap, program_proposal[i], program[i]);
		...
		current_len = ifthen(swap, proposed_len, current_len);
	}
