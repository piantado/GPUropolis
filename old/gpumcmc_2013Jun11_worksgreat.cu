...
const int N = 10000;  // oHw many chains?
...
__constant__ const float OUTPUT[DLEN] = { 1.0000000, 0.7071068, 0.5773503, 0.5000000, 0.4472136, 0.4082483, 0.3779645}; // x**-0.5

// __constant__ const float OUTPUT[DLEN] = { 0.0, 0.4041569,0.9990636,1.5557204,2.0408044,2.4605594,2.8264565}; // log(1+x*(x-1)) / (exp(2/x))

const float LL_SIGMA     = 0.01;
const float PRIOR_TEMPERATURE = .1000;

const int MCMC_ITERATIONS = 1000; // 6000000 = 7hrs; //50000;
...
// const        string FNAMES[NUM_OPS] = {  "0", "1", "x", "PI", "E", 
	...
	for(int mcmci=0;mcmci<MCMC_ITERATIONS;mcmci++) {
		...
		for(int i=0;i<2;i++) { // for each kind of proposal
			...
			if(i==0) { // a normal tree regeneration proposal
				...
				fb = (-log(current_len)-proposed_len*log(float(NUM_OPS))) - (-log(proposed_len)-current_len*log(float(NUM_OPS)));
			}
				...
				fb = (-log(float(len)) - log(float(NUM_OPS)) ) - (-log(float(len+1)) );
				...
				fb = (-log(float(len)) ) - (-log(float(len-1)) - log(float(NUM_OPS)) );
