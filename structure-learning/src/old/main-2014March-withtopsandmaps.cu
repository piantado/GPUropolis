...
hypothesis* host_hypotheses;	
int host_hypotheses_index=0;
int add_hypothesis_to_host_hypotheses( hypothesis* h ) {
	if( host_hypotheses_index >= N) return 0; // can't keep going!
	...
	if( (!found_X) || (!found_CONSTANT) ) return 1; // keep going, but don't add
		
	// else add:		
	memcpy( (void*)&(host_hypotheses[host_hypotheses_index]), (void*)h, sizeof(hypothesis) );
	host_hypotheses_index++;
...
// 	float P_2arg  = P / float(count_args[2]);
	...
	host_hypotheses = new hypothesis[N]; 
	hypothesis* host_hypothesis_tmp = new hypothesis[N]; 
	hypothesis* host_out_MAPs = new hypothesis[N];

	// initialize the hypotheses -- adding to host_hypotheses
	enumerate_all_programs( N, 7, add_hypothesis_to_host_hypotheses);
	dump_to_file(H0_PATH.c_str(), host_hypotheses, 0,0, N, 0);
	
	
	// and set all the check bits
	for(int rank=0; rank<N; rank++){
		host_hypotheses[rank].check0 = CHECK_BIT;
		host_hypotheses[rank].check1 = CHECK_BIT;
		host_hypotheses[rank].check2 = CHECK_BIT;
		host_hypotheses[rank].check3 = CHECK_BIT;
		host_hypotheses[rank].check4 = CHECK_BIT;
		host_hypotheses[rank].check5 = CHECK_BIT;
		host_hypotheses[rank].check6 = CHECK_BIT;
		
		for(int i=0;i<MAX_CONSTANTS;i++) {
			host_hypotheses[rank].constants[i] = random_normal(RNG_ARGS);
			host_hypotheses[rank].constant_types[i] = UNIFORM;
		}
	}
	...
	for(int n=0;n<N;n++){
		...
		// initialize using host_hypothesse
		COPY_HYPOTHESIS( &(host_mcmc_results[n].MAP), &(host_hypotheses[n]) );
		host_mcmc_results[n].MAP.posterior = -99e99; /// set this so it gets overwritten
		COPY_HYPOTHESIS( &(host_mcmc_results[n].sample), &(host_hypotheses[n]) );
	}
	...
	for(int outer=0;outer<OUTER_BLOCKS+BURN_BLOCKS;outer++) {
		...
		// set_MAX_PROGRAM_LENGTH(min(outer,MAX_PROGRAM_LENGTH));
		...
		for(int i=0;i<N;i++) {
			...
			host_spec[i].initialize = 0; // (outer==0)||(END_OF_BLOCK_ACTION==1);
			host_spec[i].rng_seed = seed + rng_seed*outer*N; // set this seed
